 ...
 ***************************************************************/
	...
	t_pargs pa [] = {
		...
		{"-trotter", FALSE, etBOOL, {&trotter},
		 "No trotter expansion on the propagator"},
		};

	// The program description
	const char *desc[] = {"NB: 1. OpenMP parallel computation is not yet implemented in static averaging and time-averaging approximation scheme and Trotter expansion."};
 ...
 ***************************************************************************************/
	...
	while( (!error) & (!alldone) ) {
		...
		for(fr=readframe; fr<fr_upper; fr++) {
			...
			if( (do2d) ) for(i=0; i<3; i++) gen_dip_2Q(Dip1QMat[i][xfr], Dip2QMat[xfr][i], nosc, n2Q);
		}
 ...
 *****************************************************/ 
			...
			while( (fr!=-2) && (!error) ) {
				...
				if( (fr>=0) ) {
					...
					gen_avg_1Q_ham(Ham1QMat, Ham1QAr[nAvgd], hann, nosc, fr, whann, window, nbuffer);
