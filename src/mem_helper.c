...
int graceful_exit_new( int error, int nbuffer, int win2d, int nthreads, int npol, int nise, int nosc) {
  ...
  // cjfeng 05/07/2018 
  // 2Q file pointers
  if(hfp2Q!=NULL) fclose(hfp2Q);
  for(i=0; i<3; i++) if(Dfp2Q[i]!=NULL) fclose(Dfp2Q[i]);
  ...
  // cjfeng 05/07/2018
  // Ham2QMat
  unset_gnreal_2d_array(Ham2QMat, nbuffer);
...
int set_Dip(Dip *dip, int n, int nbuffer) {
  ...
  int i;
  for(i=0; i<3; i++) {
    set_gnreal_2d_array(&(dip->Mat[i]), dip->nbuffer, dip->n);
  }
  return 0;
}
...
int unset_Dip(Dip *dip) {
  int i;
  for(i=0; i<3; i++) {
    unset_gnreal_2d_array(dip->Mat[i], dip->nbuffer);
  }
  return 0;  
}
